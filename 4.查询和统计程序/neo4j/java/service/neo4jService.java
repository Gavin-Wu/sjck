...
//            System.out.println(rates);
    ...
    public Integer getActorByGenre(String name1,String name2){
        Integer num = 0;
        try(Session session = driver.session()) {
            StatementResult result = session.run("match (g:Genre)-[]-(a:Actor) where g.name=$name1 and a.name=$name2 return count(m)"
...
//                System.out.println(result.next().fields().get(0));
    ...
    public Integer getDirectorByGenre(String name1,String name2){
        Integer num = 0;
        try(Session session = driver.session()) {
            StatementResult result = session.run("match (g:Genre)-[]-(d:Director) where g.name=$name1 and d.name=$name2 return count(m)"
                    , parameters("name1",name1,"name2",name2));
            while(result.hasNext()){
//                System.out.println(result.next().fields().get(0));
                num = result.next().fields().get(0).value().asInt();
    ...
    public Integer getDirectorByActor(String name1,String name2){
        Integer num = 0;
        try(Session session = driver.session()) {
            StatementResult result = session.run("match (d:Director)-[]-(a:Actor) where d.name=$name1 and a.name=$name2 return count(m)"
                    , parameters("name1",name1,"name2",name2));
            while(result.hasNext()){
//                System.out.println(result.next().fields().get(0));
                num = result.next().fields().get(0).value().asInt();
